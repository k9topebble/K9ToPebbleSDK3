...
static int s_bodySize = 0;
static int s_bodyOffset = 0;
...
static void _processKeys()
{
	...
	K9_APP_LOG(APP_LOG_LEVEL_DEBUG, "_processKeys found", s_messageCount);
}
#endif
...
bool _isBodyLoaded()
{
	if (s_bodySize > 0)
	{
		return s_bodyOffset >= s_bodySize;
	}
	return false;
}
...
bool _isBodyComplete(DictionaryIterator *body)
{
	...
	K9_APP_LOG(APP_LOG_LEVEL_DEBUG, "target size %d", bodySize);
	...
	// ok, got something to work with, is it complete
	{
		...
		while (content)
		{
			...
			K9_APP_LOG(APP_LOG_LEVEL_DEBUG, "chunk size %d", content->length);
			content = dict_find(body, pos++);
		}

		if (gotSize < bodySize)
		{
			K9_APP_LOG(APP_LOG_LEVEL_DEBUG, "got body/size %d/%d", gotSize, bodySize);
		...
		while (content)
		{
			int clippedLength = content->length;
			if (content->length + s_bodyOffset > MAX_BODY_SIZE)
			{
				clippedLength = MAX_BODY_SIZE - s_bodyOffset;
			}
			memcpy(s_bodyExtract + s_bodyOffset, content->value->cstring, clippedLength );
			s_bodyOffset += content->length - 1; // -1 so the next block overwrites the null at the end of this block
			content = dict_find(body, pos++);
		}
...
bool keyComplete(int key, DictionaryIterator *received)
{
	...
	/*if (res)
	{
		K9_APP_LOG(APP_LOG_LEVEL_DEBUG, "Key complete %d", baseKey);
	}*/
	return res;
}
...
void addDataToMsgs(Tuple * tuple)
{
	...
	K9_APP_LOG(APP_LOG_LEVEL_DEBUG, "msg key %lu, id %d,  and offset %d", tuple->key, message_id, offset);
	...
	K9_APP_LOG(APP_LOG_LEVEL_DEBUG, "should we refresh the screen? %d %d", message_id, s_messageCount);
...
bool _addToBody(DictionaryIterator *body)
{
	// do we have the total size?  if not extract it.
	// ok, got something to work with, is it complete
	// key 1 contains the overall length.
	// then iterate over the following keys to get the amount that's received.
	// if it's complete, copy into the body buffer
	...
	if (s_bodySize == 0)
	{
		...
		if (size)
		{
			if (size->type == TUPLE_UINT && size->length == 2)
			{
				s_bodySize = size->value->uint16;
				K9_APP_LOG(APP_LOG_LEVEL_DEBUG, "got size %d", s_bodySize);
			}
			else
			{
				K9_APP_LOG(APP_LOG_LEVEL_DEBUG, "bad command type %d", size->type);
	...
	// now look for the content in this mail, it will be of id 10 or above

	Tuple* tup = dict_read_first(body);
	while (tup)
	{
		if (tup->key > 1)
		{
			...
			gotSize += tup->length;
			K9_APP_LOG(APP_LOG_LEVEL_DEBUG, "chunk size %d", tup->length);

			int clippedLength = tup->length;
			if ((tup->length + s_bodyOffset) > MAX_BODY_SIZE)
			{
				clippedLength = MAX_BODY_SIZE - s_bodyOffset;
				K9_APP_LOG(APP_LOG_LEVEL_DEBUG, "Clipping to %d", clippedLength);
			}

			memcpy(s_bodyExtract + s_bodyOffset, tup->value->cstring, clippedLength );
			s_bodyOffset += clippedLength - 1; // -1 so the next block overwrites the null at the end of this block
			K9_APP_LOG(APP_LOG_LEVEL_DEBUG, "Got %d, %d", s_bodyOffset, s_bodySize);
			return true;
		}
		tup = dict_read_next(body);
	}
	s_bodyExtract[MAX_BODY_SIZE - 1] = 0; // ensure it's null terminated
	K9_APP_LOG(APP_LOG_LEVEL_DEBUG, "Got %d, %d", s_bodyOffset, s_bodySize);
	return false;
...
void my_in_rcv_handler(DictionaryIterator *received, void *context) {
	K9_APP_LOG(APP_LOG_LEVEL_DEBUG, "my_in_rcv_handler");
	Tuple *command = dict_find(received, 0);

	if (command)
	{
		K9_APP_LOG(APP_LOG_LEVEL_DEBUG, "Msg command %u", command->value->uint8);
		...
		case eMTR_Body:
			...
			//K9_APP_LOG(APP_LOG_LEVEL_DEBUG, "Got body")
			_addToBody(received);
			if(_isBodyLoaded())
			{
				K9_APP_LOG(APP_LOG_LEVEL_DEBUG, "Body done");
				ReadRefresh();
			}
			break;
		case eMTR_Config:
		...
		default:
			{
				K9_APP_LOG(APP_LOG_LEVEL_DEBUG, "Got unknown %d", command->value->uint8);
			}
			break;
		}
	}
